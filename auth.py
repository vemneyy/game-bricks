...
def auth_func(main_panel):
    ...
    def func_auth_success():
        input_username = username.get()
        auth_page.withdraw()
        main_game(input_username, main_panel)
    ...
    def login():
        ...
        user = sql.execute("SELECT * FROM users WHERE username = ? AND password = ?",
        ...
        if user:
            # Если пользователь найден, переходим к функции func_auth_success
            func_auth_success()
        else:
...
def reg_func(main_panel):
    ...
    def final():
        ...
        if not is_valid_password(password_final):
